...
export default function LiveLeaderboard() {
  ...
  const leaderboardUsers: LeaderboardUser[] = [
    { rank: 1, name: 'SatoshiBunny', avatar: '🐰', volume: '$1.42M', level: 42, badge: 'Crown Apex' },
    { rank: 2, name: 'NeonViper', avatar: '⚡', volume: '$980K', level: 36, badge: 'Master Trader' },
    { rank: 3, name: 'ZeroGasChad', avatar: '💎', volume: '$840K', level: 31, badge: 'DeFi Legend' },
    { rank: 4, name: 'QuantumKash', avatar: '🤖', volume: '$620K', level: 28, badge: 'AI Strategist' },
    { rank: 5, name: 'You (Guest)', avatar: '✨', volume: '$14.2K', level: 7, badge: 'Rising Star' },
  ];
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-400">Weekly guild rewards & ranking</p>
            </div>
          </div>
          ...
          <div className="flex bg-neutral-900 border border-neutral-800 p-1 rounded-xl">
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-bold transition-colors ${
                activeTab === 'quests' ? 'bg-[#9ae600] text-black' : 'text-neutral-400 hover:text-white'
              }`}
            >
              Quests
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-bold transition-colors ${
                activeTab === 'leaderboard' ? 'bg-[#9ae600] text-black' : 'text-neutral-400 hover:text-white'
              }`}
            >
              Ranks
            </button>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <h4 className="text-xs sm:text-sm font-semibold text-neutral-200">{q.title}</h4>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs font-bold text-white flex items-center gap-1.5">
                      <span>{user.name}</span>
                      <span className="text-[10px] text-neutral-400 font-normal">LVL {user.level}</span>
                    </div>
                    <span className="text-[10px] text-neutral-400 font-sans">{user.badge}</span>
                  </div>
                </div>

                <div className="text-right">
                  <div className="text-xs font-sans font-bold text-[#a3e635]">{user.volume}</div>
                  <span className="text-[10px] text-neutral-400 font-medium">Vol.</span>
                </div>
              </div>
            ))}
          </div>
